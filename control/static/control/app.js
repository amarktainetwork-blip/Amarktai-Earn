(function () {
  ...
  function renderMoney(data) {
    ...
    const settled = cardValue(overview.cards, "SETTLED 30D", "$0.00"); const pending = cardValue(overview.cards, "PENDING PAYOUT", "$0.00"); const costCard = (overview.cards || []).find((row) => String(row.label || "").includes("PAID EXECUTION COST 30D")); const cost = costCard ? costCard.value : "$0.00"; const coverageComplete = overview.meta && overview.meta.settled_profit_cost_coverage_complete === true; const profitCard = (overview.cards || []).find((row) => String(row.label || "").includes("NET SETTLED PROFIT 30D")); const profit = profitCard ? profitCard.value : "$0.00"; const profitLabel = coverageComplete ? "TRUE RECORDED NET SETTLED PROFIT · 30D" : "RECORDED NET SETTLED PROFIT · COST COVERAGE INCOMPLETE";
