(function () {
  ...
  function renderLaunchReadiness(overview, markets, alerts, jobs) {
    const unknownRemote = Number(String(cardValue(overview.cards, "UNKNOWN REMOTE STATE", "0")).replace(/\D/g, "")) || 0;
    const failedJobs = Number(String(cardValue(overview.cards, "FAILED JOBS", "0")).replace(/\D/g, "")) || 0;
    const readyMarkets = markets.filter(marketReady).length;
    const openActions = (alerts || []).filter((row) => !["RESOLVED", "ACKNOWLEDGED"].includes(row.status)).length;
    const settled = (jobs || []).some((row) => row.state === "SETTLED");
    const engineeringReady = unknownRemote === 0 && failedJobs === 0;
    const steps = [
      [engineeringReady, "Engineering proof", engineeringReady ? "Recovery and runtime evidence are clear" : "Resolve failed or unknown work before launch", "/ops/jobs/"],
      [readyMarkets > 0, "Payout-ready connection", readyMarkets > 0 ? `${readyMarkets} connection${readyMarkets === 1 ? " is" : "s are"} ready for verified work` : "Verify one South African fiat payout route", "/ops/markets/"],
      [openActions === 0, "Owner actions", openActions === 0 ? "No launch actions are waiting" : `${openActions} owner action${openActions === 1 ? "" : "s"} still require attention`, "/ops/alerts/"],
      [settled, "First settled sale", settled ? "A commercial payout has been reconciled" : "Complete one bounded paid order and settlement", "/ops/money/"],
    ];
    const complete = steps.filter(([ready]) => ready).length;
    return `<section class="launch-readiness"><div class="launch-readiness-head"><div><h2>Path to first revenue</h2><p>${complete} of ${steps.length} launch gates complete. Work from left to right; autonomy stays off until the first settled proof.</p></div>${stateBadge(complete === steps.length ? "READY" : "OWNER_ACTION_REQUIRED")}</div><div class="launch-readiness-grid">${steps.map(([ready, title, copy, href], index) => `<a class="launch-step ${ready ? "ready" : ""}" href="${href}"><i>${ready ? "✓" : index + 1}</i><span><strong>${esc(title)}</strong><small>${esc(copy)}</small></span></a>`).join("")}</div></section>`;
  }
  ...
  function renderOverview(data) {
    ...
    root.innerHTML = `<section class="overview-welcome"><div><span class="eyebrow">OWNER OVERVIEW</span><h2>${greeting}</h2><p>What can earn, what is blocked, and what needs your attention.</p></div><div class="autonomy-pill ${modeClass(mode)}"><small>AUTONOMY STATE</small><strong>${esc(human(mode))}</strong></div></section>
      ${renderLaunchReadiness(overview, markets, alerts, jobs)}
  ...
  function renderServices(data) {
    ...
    root.innerHTML = `<div class="page-toolbar"><div class="page-toolbar-copy"><h2>Products you can sell</h2><p>See what is ready, what still needs setup, where it is listed, and whether orders have arrived.</p></div></div><div class="health-grid"><article class="health-card"><small>PRODUCTS</small><strong>${offerings.length}</strong><p>Available offers</p></article><article class="health-card"><small>READY TO SELL</small><strong>${offerings.filter((row) => row.sellable).length}</strong><p>All required checks passed</p></article><article class="health-card"><small>LIVE LISTINGS</small><strong>${listingCount}</strong><p>Published sales pages</p></article><article class="health-card"><small>ORDERS</small><strong>${orderCount}</strong><p>Orders received</p></article></div>${body}<details class="ledger-disclosure"><summary>Product workshop · ${products.length} ideas and products</summary><div class="panel-body"><p class="settings-copy">${esc(factory.truth || "Products are not counted as income until a payment settles.")}</p>${products.length ? genericTable(products) : empty("No owned products", "No product ideas have been saved yet.")}</div></details>`;
  }
  ...
  function renderMoney(data) {
    ...
    root.innerHTML = `<div class="page-toolbar"><div class="page-toolbar-copy"><h2>Your money</h2><p>Only received payouts count as cash. Pending payouts, fees, costs, and profit stay separate.</p></div></div><div class="health-grid finance-grid">${finance.map(([label, value, truth]) => `<article class="health-card"><small>${esc(label)}</small><strong>${esc(value)}</strong><p>${esc(truth)}</p></article>`).join("")}</div><div class="section-grid"><section class="panel"><div class="panel-head"><div><h2>Payout movement</h2><p>Real payout history only; no invented chart points</p></div></div><div class="panel-body chart-shell">${buildChart(earnings)}</div></section><section class="panel"><div class="panel-head"><div><h2>Balances</h2><p>Earned, pending, and received remain separate</p></div></div>${treasury.rows.length ? genericTable(treasury.rows) : empty("No balances yet", "Balances appear after a real payment moves through its lifecycle.")}</section></div>${panel("Recent payouts", "Gross amount, fee, net amount, payment state, and reference", earnings.length ? genericTable(earnings) : empty("No payout history yet", "Real job payouts will appear here without being counted as cash before settlement."))}${panel("Profit breakdown", "Actual profitability by source, work type, and period", dimensions.length ? genericTable(dimensions) : empty("No profitability aggregates yet", "This view stays empty until real settled performance exists."))}<details class="ledger-disclosure"><summary>Advanced accounting ledger</summary>${treasury.secondary_rows && treasury.secondary_rows.length ? genericTable(treasury.secondary_rows) : empty("No advanced ledger activity has been recorded.")}</details>`;
  }
  ...
  function renderMarkets(data) {
    ...
    root.innerHTML = `<div class="page-toolbar"><div class="page-toolbar-copy"><h2>Connect places to sell and get paid</h2><p>Complete one account at a time: connect it, prove it can receive work, and verify where your payout will arrive.</p></div></div><div class="health-grid"><article class="health-card"><small>AVAILABLE CONNECTIONS</small><strong>${Number(meta.total || accounts.length)}</strong><p>Supported accounts</p></article><article class="health-card"><small>CONNECTED</small><strong>${Number(meta.connections_verified || 0)}</strong><p>Connection test passed</p></article><article class="health-card"><small>PAYOUT READY</small><strong>${Number(meta.payout_routes_proven || 0)}</strong><p>Receipt route verified</p></article><article class="health-card"><small>AUTOMATION</small><strong>${esc(human(meta.autonomy_state || "OFF"))}</strong><p>Kept off during launch proof</p></article></div>${accounts.length ? `<div class="account-groups">${accountCards}</div>` : empty("No connections available", "Set up the supported account catalogue first.")}${marketRows}<div class="drawer" id="accountDrawer" hidden><aside class="drawer-panel" role="dialog" aria-modal="true" aria-labelledby="accountDrawerTitle"><div id="accountDrawerContent"></div></aside></div>`;
