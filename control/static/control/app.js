(function () {
  ...
  function marketReady(row) {
    const channels = row.revenue_channels || [];
    const serviceChannel = channels.includes("SERVICE_LISTING") || channels.includes("PAY_PER_CALL_API");
    const channelReady = serviceChannel ? row.seller_listing_ready === true : row.discovery_ready === true;
    return Boolean(row.enabled && row.status === "LIVE" && row.connected === true && channelReady && row.payout_ready && row.south_africa_verified && row.policy_current === true && row.autonomy_allowed === true && !readableBlockers(row).length);
  }
  function firstBlocker(row) { return readableBlockers(row)[0] || (marketReady(row) ? "Ready for verified work" : "Channel readiness evidence is incomplete"); }
  ...
  function renderMarkets(data) {
    ...
    root.innerHTML = `<div class="page-toolbar"><div class="page-toolbar-copy"><h2>Revenue channel readiness</h2><p>Job sources, service channels, bounties, manual storefronts, and off-host candidates remain visibly distinct.</p></div></div>${rows.length ? `<div class="market-grid">${rows.map((row) => { const ready = marketReady(row); const items = [["Connected", row.connected === true], ["Discovery ready", row.discovery_ready === true], ["Seller listing ready", row.seller_listing_ready === true], ["Payout ready", row.payout_ready], ["South Africa verified", row.south_africa_verified], ["Policy current", row.policy_current === true], ["Autonomy allowed", row.autonomy_allowed === true]]; return `<article class="market-card"><div class="market-card-head"><span class="market-logo">${esc(row.market.slice(0, 2))}</span><div class="market-card-title"><small>${esc(row.category || "JOB SOURCES")}</small><h3>${esc(human(row.market))}</h3><p>${esc(row.settlement_rail || row.payout_method || "Payout method not verified")}</p></div>${stateBadge(row.status)}</div><div class="market-verdict ${ready ? "ready" : ""}">${esc(ready ? "READY FOR VERIFIED WORK" : firstBlocker(row))}</div><div class="readiness-list">${items.map(([label, yes]) => `<div class="readiness-item ${yes ? "yes" : ""}"><i>${yes ? "✓" : "·"}</i>${esc(label)}</div>`).join("")}</div><div class="market-stats"><div><strong>${Number(row.opportunities_seen_24h || 0)}</strong><small>24H SEEN</small></div><div><strong>${Number(row.applications_total || 0)}</strong><small>APPLICATIONS</small></div><div><strong>${Number((row.service_listings || {}).PUBLISHED || 0)}</strong><small>LIVE LISTINGS</small></div><div><strong>${esc(row.settled_net || "0.00")}</strong><small>SETTLED VALUE</small></div></div>${!ready ? `<div class="market-blocker">Next: ${esc(firstBlocker(row))}</div>` : ""}<details class="ledger-disclosure"><summary>Advanced integration detail</summary><div class="panel-body"><div class="drawer-field"><small>REVENUE CHANNELS</small><strong>${esc((row.revenue_channels || []).join(", ") || "Posted work")}</strong></div><div class="drawer-field"><small>HOSTING POLICY</small><strong>${esc(row.hosting_policy || "UNVERIFIED")}</strong></div><div class="drawer-field"><small>API CONTRACT</small><strong>${esc(row.api_contract_state || "UNVERIFIED")}</strong></div><div class="drawer-field"><small>POLICY CHECKED</small><strong>${esc(row.policy_checked || "Not verified")}</strong></div></div></details></article>`; }).join("")}</div>` : empty("No revenue channels configured", "Bootstrap disabled catalog records before any external enablement.")}`;
  }
