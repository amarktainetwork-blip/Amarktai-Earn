    ...
    try:
        with transaction.atomic():
            worker, _ = Worker.objects.select_for_update().get_or_create(
                id=worker_id,
                defaults={"worker_class": "structured_data", "node": node_id, "status": "READY"},
            )
            if worker.worker_class != "structured_data":
                raise ExecutionError("worker is not a structured-data worker")
        ...
        if not result.ok:
            Execution.objects.filter(pk=execution.pk).update(
                ...
                error_detail=(result.error or "structured data worker failed")[:4000],
            )
            ...
            raise ExecutionError(result.error or "structured data worker failed")
        ...
        primary = Path(artifact_rows[0].path)
        qa = verify_csv(
            primary,
            expected_rows=result.evidence.get("rows") if isinstance(result.evidence, dict) else None,
            required_columns=result.evidence.get("columns") if isinstance(result.evidence, dict) else None,
        )
        ...
        Execution.objects.filter(pk=execution.pk).update(
            status=status,
            ended_at=timezone.now(),
            result={"worker_evidence": result.evidence, "qa": {"passed": qa.passed, "checks": qa.checks}},
        )
        ...
        AuditEvent.objects.create(
            ...
            actor="qa-deterministic",
            metadata={"job_id": str(job.id), "execution_id": execution.id, "checks": qa.checks},
        )
