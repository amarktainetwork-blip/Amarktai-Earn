...
DEFINITIONS = (
    IntegrationDefinition("paystack", "Paystack", "DIRECT_COMMERCE", "Owned checkout and authoritative payment receipt", "ACTIVE_PROVING", (_field("secret_key", "Secret key"), _field("public_key", "Public key", required=False, public_identifier=True)), "PAYSTACK", ("CONNECTION_TEST", "DIRECT_CHECKOUT", "WEBHOOKS", "ORDER_INTAKE", "PAYMENT_RECONCILIATION"), (), "PAYSTACK_BALANCE", False, webhook_supported=True, order_intake="AUTOMATED_AFTER_PROOF", owner_action="Open the merchant account, finish KYC, create test/live keys, and configure the Paystack webhook externally."),
    ...
    IntegrationDefinition("valr", "VALR", "TREASURY_RAIL", "View-only crypto receipt and conversion evidence", "CONNECTION_READY", (_field("api_key", "View-only API key"), _field("view_api_secret", "View-only API secret")), "VALR", ("CONNECTION_TEST", "BALANCE_READ", "RECEIPT_RECONCILIATION"), ("CONVERSION", "HUMAN_WITHDRAWAL"), "CRYPTO_RECEIVED", True, order_intake="NOT_APPLICABLE", owner_action="Create a VALR API key restricted to View permissions; never grant withdrawal permission."),
    ...
    IntegrationDefinition("taskbounty", "TaskBounty Solver", "BOUNTY_MARKET", "API-native task discovery and submission", "ACTIVE_PROVING", (_field("api_key", "Solver API key"), _field("solana_usdc_address", "Public Solana/USDC address", required=False, public_identifier=True), _field("eth_address", "Public ETH address", required=False, public_identifier=True), _field("btc_address", "Public BTC address", required=False, public_identifier=True)), "TASKBOUNTY", ("CONNECTION_TEST", "DISCOVERY", "QUALIFICATION", "ACQUISITION", "SUBMISSION", "STATUS"), ("PAYOUT_PROOF",), "CRYPTO_RECEIVED", True, order_intake="AUTOMATED_AFTER_PROOF", owner_action="Create the solver key and enter only a supported public payout address; then prove an on-chain receipt."),
    ...
    IntegrationDefinition("freelancer", "Freelancer.com", "PROJECT_MARKET", "Official-API project work where supported", "CONNECTION_READY", (_field("oauth_token", "OAuth access token"),), "MANUAL", (), ("API_CONTRACT_REVIEW", "PROPOSAL_SUBMISSION", "MESSAGE_HANDLING", "PAYOUT_PROOF"), "PAYPAL_OR_PAYONEER", True, order_intake="OWNER_ASSISTED", owner_action="Create an official API application/token and finish KYC; unsupported actions remain explicit owner steps."),
    ...
    IntegrationDefinition("nevermined", "Nevermined", "STABLECOIN_COMMERCE", "Future stablecoin agent/API payments", "INACTIVE", (_field("api_key", "API key"), _field("public_receiving_address", "Public stablecoin receiving address", public_identifier=True)), "MANUAL", (), ("AUTHORITATIVE_PLAN_CONTRACT_REVIEW", "PLAN_PUBLICATION", "PAYMENT_PROOF"), "STABLECOIN_RECEIVED", True, order_intake="BLOCKED", owner_action="Complete provider onboarding and supply the current official Plan/payment contract plus a public receiving address; the route stays blocked until that external contract and receipt are proven.", off_host_requirements=("NO_WALLET_SIGNING_ON_WEBDOCK",)),
